...
def valid(model, valloader, input_size, num_samples, gpus):
    ...
    with torch.no_grad():
        loop = tqdm(valloader, position = 0, leave = True)
        for index, batch in enumerate(valloader):
            image, meta = batch
            num_images = image.size(0)
            if index % 100 == 0:
                print('%d  processed' % (index * num_images))
    ...
    print(f'Parsing preds: {parsing_preds.shape}')

    return parsing_preds, fused_img, scales, centers
